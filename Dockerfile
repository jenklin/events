...
# GALLERY_ORIGIN must be present at BUILD time: Next bakes rewrites() into the
# routes manifest during `next build`, so a runtime-only value would no-op.
ARG GALLERY_ORIGIN

ENV NEXT_PUBLIC_SUPABASE_URL=$NEXT_PUBLIC_SUPABASE_URL
ENV NEXT_PUBLIC_SUPABASE_ANON_KEY=$NEXT_PUBLIC_SUPABASE_ANON_KEY
ENV SUPABASE_SERVICE_ROLE_KEY=$SUPABASE_SERVICE_ROLE_KEY
ENV GALLERY_ORIGIN=$GALLERY_ORIGIN
